<template>
	<view class="imageUploadContainer">
		<view class="imageUploadList">
			...
			<view v-if="isShowAdd && $api.my('role_id')>3" class="imageUpload" @tap="selectImage">+</view>
		</view>
		<image v-if="showMoveImage" class="moveImage" :style="{left:posMoveImageLeft, top:posMoveImageTop}" :src="moveImagePath"></image>		 
		  <button @click="sub" type="default" v-if="$api.my('role_id')>3">上传</button>
		  
		<uni-popup :show="edit_box" type="bottom" mode="fixed" @hidePopup="hidePopup">
			<view class="uni-list">
				<view class="build_01">		
				修改名称：				
					<input v-model="e_data.name" type="text" />
				</view> 
				<view class="popup_foot">
					<view class="popup_foot_l" @click="edit_box=false">取消</view>
					<view class="popup_foot_r" @click="sub_name()">确认</view>
				</view>
			</view>
		</uni-popup>  
	</view>
	...
	export default {
		...
		methods:{
			...
			sub_name(){
				...
				this.$api.http.post('cpy/plan_edit_name',data).then(res=>{
					if(res){
						this.$api.msg('操作成功')
					}
				})

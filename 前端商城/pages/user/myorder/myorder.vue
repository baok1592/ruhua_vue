<template>
	<view>
		...
		<view class="goods-section">
			...
			<!-- 商品列表 -->
			<view class="g-item" v-for="(item,index) of my_order.ordergoods" @click="jump_pro(item.goods_id)">
				<image :src="getimg+item.imgs.url"></image>
				<view class="right">
		...
		<view class="btn" v-if="my_order.payment_state==1">
			<view class="btn_1"  v-if="my_order.state==0" @click="jump_torefund(my_order.order_id)">退款</view>
			<view class="btn_1" v-if="my_order.state==1" @click="jump_tograde">评分</view>
		</view>
	</view>
</template>
...
<script>
	export default {
		...
		methods: {
			...
			// }, 
			get_my_order(id){
				this.$api.http.get('order/user/get_order_one',{id:id}).then(res=>{
